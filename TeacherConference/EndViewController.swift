...
class EndViewController: UIViewController,MFMailComposeViewControllerDelegate {
    ...
    @IBAction func printData(sender: UIBarButtonItem) {
        var fileData = "\(studentNameTextField.text) \(parentNameTextField.text) \(parentEmailTextField.text) \(teacherNameTextField.text) \(teacherEmailTextField.text) \(courseTextField.text) \(timeTextField.text)"
